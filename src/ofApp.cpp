		...
		for (int y=0; y<h; ++y) {
			for (int x=0; x<w; ++x) {
				if (depth[x + w * y] != 0 && depth[x + w * y] < 8000 && depth[x + w * y] == depth[x + w * y]) {
					...
					if(x >= w / 2.0)
						*point = (w/2.0 + cos(pixel_base_ang[x + w * y]) * actual_pixel_base + abs_width[x + w * y]) / 1.0;
					else
						*point = (w/2.0 - (cos(pixel_base_ang[x + w * y]) * actual_pixel_base + abs_width[x + w * y])) / 1.0;

					if(y >= h / 2.0)
						*(point + 1) = (h/2.0 + obj_height) / 1.0;
					else
						*(point + 1) = (h/2.0 - obj_height) / 1.0;
					float z =  sin(pixel_base_ang[x + w * y]) * actual_pixel_base;
					*(point + 2) = -z;
